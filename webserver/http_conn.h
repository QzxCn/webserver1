...
public:
    ...
    static const int READ_BUFFER_SIZE = 10000;   // 读缓冲区的大小
    static const int WRITE_BUFFER_SIZE = 10000;  // 写缓冲区的大小
...
public:
    ...
    void tick() {
        cout<<"tick()";
        if( !head ) {
            cout<<"没有找到打开的连接，即定时器链表无内容"<<endl;
            return;
        }
        
        time_t cur = time( NULL );  // 获取当前系统时间
        util_timer* tmp = head;
        cout<<"检查节点是否超时，当前的头节点连接为" ;
        cout<<head->user_data->m_sockfd<<endl;
